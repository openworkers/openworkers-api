<script lang="ts">
  // OpenWorkers brand mark. Not a lucide icon (no equivalent exists), so it
  // lives as a shared component instead of inline SVG duplicated per page.
  let { class: className = 'size-6 fill-foreground' }: { class?: string } = $props();
</script>

<svg viewBox="0 0 283.46 283.46" class={className} aria-label="OpenWorkers">
  <path
